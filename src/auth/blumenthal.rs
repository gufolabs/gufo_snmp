...
// BS  - HMAC block size
pub struct DigestAuthWithBlumenthal<
    D: Digest,
    const KS: usize,
    const SS: usize,
    const PKS: usize,
...
impl<D: Digest, const KS: usize, const SS: usize, const PKS: usize, const BS: usize> Default
    for DigestAuthWithBlumenthal<D, KS, SS, PKS, BS>
{
    fn default() -> Self {
        Self {
            key: [0; KS],
...
impl<D: Digest, const KS: usize, const SS: usize, const PKS: usize, const BS: usize>
    DigestAuthWithBlumenthal<D, KS, SS, PKS, BS>
{
    fn expand(&mut self) {
        self.expanded_key[..KS].copy_from_slice(&self.key);
        let mut current = [0u8; PKS];
        current[..KS].copy_from_slice(&self.key);
        let mut current_len = KS;
        let mut offset = KS;
        while offset < PKS {
            ...
            offset += n;
            if current_len + digest.len() <= PKS {
                current[current_len..current_len + digest.len()].copy_from_slice(&digest);
                current_len += digest.len();
            }
        }
    }
