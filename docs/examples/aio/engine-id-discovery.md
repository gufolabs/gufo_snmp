...
--8<-- "examples/aio/engine-id-discovery.py"
```
...
``` py title="engine-id-discovery.py" linenums="1" hl_lines="4 5"
--8<-- "examples/aio/engine-id-discovery.py"
```

`SnmpSession` object holds all necessary API, so import it from `gufo.snmp.aio`.
For SNMPv3 we also need a `User` class.
...
``` py title="engine-id-discovery.py" linenums="1" hl_lines="11"
--8<-- "examples/aio/engine-id-discovery.py"
```
...
``` py title="engine-id-discovery.py" linenums="1" hl_lines="14"
--8<-- "examples/aio/engine-id-discovery.py"
```
...
```
$ python3 examples/aio/engine-id-discovery.py 127.0.0.1 user1
